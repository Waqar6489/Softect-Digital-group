...
import { CheckCircle, Phone, Mail, Sparkles, ShoppingCart, ShoppingBag, BarChart3, Target } from 'lucide-react';
...
export default function EcommerceLandingPage() {
  ...
  const trustItems = [
    { name: "Fashion Stores", icon: "👗" },
    { name: "Electronics Retailers", icon: "💻" },
    { name: "Beauty & Cosmetics", icon: "💄" },
    { name: "Home & Living Stores", icon: <ShoppingBag/> },
    { name: "Health & Wellness Brands", icon: "🌿" },
    { name: "Baby & Kids Stores", icon: "🧸" },
    { name: "Online Marketplaces", icon: <ShoppingCart/> },
  ...
  const [formData, setFormData] = useState({
    ...
    businessName: '', website: '', productsCategory: '', budget: '',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${apiBase}/api/ecommerce-lead`, {
        method: 'POST',
      ...
      if (result.status === 'success') {
        ...
        setFormData({ name: '', email: '', phone: '', message: '', businessName: '', website: '', productsCategory: '', budget: '' });
        setStep(1);
    ...
    <div className="bg-gray-50 text-gray-900 font-sans antialiased">
      ...
      <section className="max-w-7xl mx-auto px-4 py-12 lg:py-20 grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
        <div className="lg:col-span-7 space-y-6">
          ...
          <h1 className="text-4xl md:text-6xl font-black uppercase tracking-tight leading-none text-[#122a52]">
            More Leads.<br />More Customers.<br /><span className="text-[#a442af]">More Revenue.</span>
          </h1>
          <p className="text-xl text-slate-700 font-medium max-w-2xl">
            Lead Generation Services for E-commerce & Online Retail Businesses using Google Ads, Meta Ads, TikTok Ads & Cross-Channel Marketing.
          </p>
          <div className="space-y-3 pt-4">
            {["Attract high-intent shoppers ready to buy","Increase website traffic, sign-ups & add-to-cart actions","Boost sales & revenue with qualified leads","Maximize ROI with data-driven ad campaigns","Real-time tracking, reporting & campaign optimization"].map((text, idx) => (
        ...
        <div id='form' className="lg:col-span-5 border-4 border-[#a442af] rounded-2xl p-6 bg-white shadow-xl">
          <div className="text-center mb-6 pt-2">
            ...
            <h2 className="text-2xl font-black uppercase tracking-tight text-[#a442af]">E-Commerce Leads</h2>
          </div>
          ...
          <form onSubmit={(e) => e.preventDefault()} className="space-y-4">
            ...
            {step === 2 && (
              <div className="space-y-4">
                {[{label:'Business Name',id:'businessName',type:'text'},{label:'Website',id:'website',type:'text'},{label:'Products / Category',id:'productsCategory',type:'text'},{label:'Monthly Ad Budget',id:'budget',type:'number'}].map((f) => (
                  <div key={f.id} className="flex flex-col sm:flex-row sm:items-center gap-2 border-b border-gray-400 pb-1">
                    <label htmlFor={f.id} className="font-bold text-sm text-[#122a52] sm:w-1/3 whitespace-nowrap">{f.label}:</label>
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100">
          <div className="lg:col-span-5 p-8 lg:p-12 space-y-6">
            <h3 className="text-[#a442af] uppercase font-black tracking-tight text-lg">Helping E-Commerce Businesses</h3>
            <h2 className="text-3xl font-black uppercase text-[#122a52] leading-tight">Grow More. <span className="text-[#a442af]">Sell More.</span></h2>
            <p className="text-slate-600 font-medium">Softech helps e-commerce & online retail brands generate high-quality leads and customers through Google Ads, Meta Ads, TikTok Ads, and Cross-Channel Marketing.</p>
            <p className="text-slate-600 font-medium">From product discovery to purchase, we help you reach the right audience, drive more traffic to your store, and turn visitors into loyal buyers.</p>
          ...
          <div className="lg:col-span-7 relative h-full min-h-[450px]">
            <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url('https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?q=80&w=1200&auto=format&fit=crop')` }}>
            ...
            <div className="absolute bottom-6 right-6 left-6 md:left-auto md:w-96 bg-white p-6 rounded-2xl shadow-2xl border-t-4 border-[#a442af] space-y-3">
              <div className="flex items-center gap-2">
                <div className="bg-[#a442af] text-white p-2 rounded-full"><ShoppingCart size={18} /></div>
                <h4 className="font-black uppercase tracking-tight text-sm text-[#122a52]">Helping E-commerce Brands Scale Faster</h4>
              </div>
              <p className="text-xs text-slate-600 font-medium leading-relaxed">We deliver high-intent leads and customers that shop more, buy more, and come back again — increasing your sales, average order value, and lifetime value.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="text-center mb-8 flex items-center justify-center gap-4">
          ...
          <h2 className="text-2xl font-black uppercase tracking-tight text-[#122a52]">What Our E-Commerce Clients Say</h2>
          <div className="hidden sm:block h-[2px] w-16 bg-[#a442af]"></div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { quote: "Softech helped us increase our website traffic by 85% and sales by 60% in just 90 days through Google & Meta Ads.", author: "TrendyKart" },
            { quote: "Our ROAS improved 3X with highly targeted campaigns on TikTok & cross-channel marketing.", author: "UrbanDeals" },
            { quote: "Great team, transparent reporting, and campaigns that bring real customers and revenue.", author: "ShopNest" }
          ].map((t, idx) => (
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="text-center mb-10 flex items-center justify-center gap-4">
          <div className="hidden sm:block h-[2px] w-16 bg-[#a442af]"></div>
          <h2 className="text-2xl font-black uppercase tracking-tight text-[#122a52]">Our E-Commerce Lead Generation Services</h2>
          <div className="hidden sm:block h-[2px] w-16 bg-[#a442af]"></div>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4 text-center">
          {[
            { label: "Google Ads for E-commerce", icon: <FaGoogle className="text-[#a442af] mx-auto" size={28} /> },
            ...
            { label: "Product Catalog & Feed Optimization", icon: <ShoppingBag className="text-[#a442af] mx-auto" size={26} /> },
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="bg-white border border-gray-200 rounded-3xl p-8 shadow-sm">
          <div className="text-center mb-8 flex items-center justify-center gap-4">
            <div className="hidden sm:block h-[2px] w-16 bg-[#a442af]"></div>
            <h2 className="text-2xl font-black uppercase tracking-tight text-[#122a52]">We Solve E-Commerce Marketing Challenges</h2>
            <div className="hidden sm:block h-[2px] w-16 bg-[#a442af]"></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
            {["Low website traffic & poor quality visitors","High ad spend with low ROAS","Cart abandonment & low conversions","Poor product visibility & ranking","High competition & rising ad costs","Lack of repeat customers","Inconsistent sales & revenue","Ineffective targeting & ad creatives"].map((c, idx) => (
      ...
      <section className="max-w-7xl mx-auto px-4 py-12 text-center">
        <style>{`@keyframes mq-ec{0%{transform:translateX(0%)}100%{transform:translateX(-50%)}}.mq-ec{display:flex;width:max-content;animation:mq-ec 32s linear infinite}.mq-ec:hover{animation-play-state:paused}`}</style>
        <h3 className="text-slate-800 uppercase font-black text-sm tracking-widest mb-6">Trusted By E-Commerce & Online Retail Brands</h3>
        <div className="relative w-full overflow-hidden py-2">
          <div className="mq-ec gap-6">
      ...
      <section className="max-w-7xl mx-auto px-4 pb-16">
        <div className="bg-[#a442af] text-white rounded-3xl p-8 md:p-12 text-center md:text-left flex flex-col md:flex-row justify-between items-center gap-6 shadow-xl">
          <div className="space-y-2">
            <h2 className="text-2xl md:text-3xl font-black uppercase tracking-tight">READY TO SCALE YOUR E-COMMERCE BUSINESS?</h2>
            <p className="font-bold text-purple-100">More Traffic. More Customers. More Sales. Book Your Free Marketing Consultation Today.</p>
          </div>
      ...
      <footer className="bg-[#122a52] text-white text-xs py-4 px-4 font-medium">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-3 text-center">
          <div className="flex flex-wrap justify-center gap-2 md:gap-4 text-slate-300">
            <span className="font-bold text-white">🛡️ E-commerce Lead Generation Experts</span>
