...
import { CheckCircle, Phone, Mail, Dumbbell, Activity, BarChart3, Users } from 'lucide-react';
...
export default function FitnessLandingPage() {
  ...
  const trustItems = [
    { name: "Gyms & Health Clubs", icon: "🏋️" },
    { name: "Yoga & Pilates Studios", icon: "🧘" },
    { name: "CrossFit & Bootcamps", icon: "💪" },
    { name: "Personal Training Studios", icon: "🎯" },
    { name: "Dance & Zumba Studios", icon: "💃" },
    { name: "Spin & Cycling Studios", icon: "🚴" },
    { name: "Martial Arts Academies", icon: "🥋" },
    { name: "Boxing & MMA Gyms", icon: "🥊" },
  ];
  ...
  const [formData, setFormData] = useState({
    ...
    gymStudioName: '', servicesOffered: '', location: '', budget: '',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${apiBase}/api/fitness-lead`, {
        method: 'POST',
      ...
      if (result.status === 'success') {
        ...
        setFormData({ name: '', email: '', phone: '', message: '', gymStudioName: '', servicesOffered: '', location: '', budget: '' });
        setStep(1);
    ...
    <div className="bg-gray-50 text-gray-900 font-sans antialiased">
      ...
      <section className="max-w-7xl mx-auto px-4 py-12 lg:py-20 grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
        <div className="lg:col-span-7 space-y-6">
          ...
          <h1 className="text-4xl md:text-6xl font-black uppercase tracking-tight leading-none text-[#122a52]">
            More Leads.<br />More Members.<br />More Results.<br /><span className="text-[#a442af]">More Growth.</span>
          </h1>
          <p className="text-xl text-slate-700 font-medium max-w-2xl">
            Lead Generation Services for Gyms, Fitness Studios & Personal Trainers Using Google Ads, Meta Ads, Instagram Ads, TikTok Ads & Cross-Channel Marketing.
          </p>
          <div className="space-y-3 pt-4">
            {["Attract high-intent gym & fitness leads","Increase trial sign-ups & memberships","Fill your classes, PT sessions & programs","Maximize ROI with data-driven campaigns","Real-time tracking, reporting & optimization"].map((text, idx) => (
        ...
        <div id='form' className="lg:col-span-5 border-4 border-[#a442af] rounded-2xl p-6 bg-white shadow-xl">
          <div className="text-center mb-6 pt-2">
            ...
            <h2 className="text-2xl font-black uppercase tracking-tight text-[#a442af]">Fitness & Gym Leads</h2>
          </div>
          ...
          <form onSubmit={(e) => e.preventDefault()} className="space-y-4">
            ...
            {step === 2 && (
              <div className="space-y-4">
                {[
                  {label:'Gym / Studio Name',id:'gymStudioName',type:'text'},
                  {label:'Services Offered',id:'servicesOffered',type:'text',placeholder:'Gym / Personal Training / Classes / Both'},
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100">
          <div className="lg:col-span-5 p-8 lg:p-12 space-y-6">
            <h3 className="text-[#a442af] uppercase font-black tracking-tight text-lg">Helping Fitness Businesses</h3>
            <h2 className="text-3xl font-black uppercase text-[#122a52] leading-tight">Get More Members, More Bookings, <span className="text-[#a442af]">More Transformations</span></h2>
            <p className="text-slate-600 font-medium">We help gyms, fitness studios and personal trainers generate qualified leads through Google Ads, Meta Ads, Instagram Ads, TikTok Ads and cross-channel marketing strategies.</p>
            <p className="text-slate-600 font-medium">Whether you run a gym, yoga studio, bootcamp, Zumba class or offer personal training – our campaigns bring you people actively looking for fitness solutions.</p>
          ...
          <div className="lg:col-span-7 relative h-full min-h-[450px]">
            <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url('https://images.unsplash.com/photo-1534438327276-14e5300c3a48?q=80&w=1200&auto=format&fit=crop')` }}>
            ...
            <div className="absolute bottom-6 right-6 left-6 md:left-auto md:w-96 bg-white p-6 rounded-2xl shadow-2xl border-t-4 border-[#a442af] space-y-3">
              <div className="flex items-center gap-2">
                <div className="bg-[#a442af] text-white p-2 rounded-full"><Dumbbell size={18} /></div>
                <h4 className="font-black uppercase tracking-tight text-sm text-[#122a52]">Delivering Fitness Leads That Convert Into Members</h4>
              </div>
              <p className="text-xs text-slate-600 font-medium leading-relaxed">We deliver high-intent enquiries that turn into trial sign-ups, class bookings, personal training sessions and long-term memberships – helping your fitness business grow stronger every month.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { quote: "Softech helped us increase trial sign-ups by 180% in 3 months. Our membership numbers have never been this high!", author: "FitZone Gym" },
            { quote: "High-quality leads, more class bookings and great ROI. Their campaigns really understand the fitness industry.", author: "PowerHouse Fitness Studio" },
            { quote: "Amazing results! More personal training enquiries and consistent new members every month. Highly recommended!", author: "Stronger You Fitness" }
          ].map((t, idx) => (
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="text-center mb-10 flex items-center justify-center gap-4">
          <div className="hidden sm:block h-[2px] w-16 bg-[#a442af]"></div>
          <h2 className="text-2xl font-black uppercase tracking-tight text-[#122a52]">Our Fitness Lead Generation Services</h2>
          <div className="hidden sm:block h-[2px] w-16 bg-[#a442af]"></div>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4 text-center">
          {[
            { label: "Google Ads for Gyms", icon: <FaGoogle className="text-[#a442af] mx-auto" size={28} /> },
            ...
            { label: "Lead Form Ads for Trial Sign-Ups", icon: <FaSitemap className="text-[#a442af] mx-auto" size={26} /> },
            { label: "Landing Page CRO & Lead Optimization", icon: <BarChart3 className="text-[#a442af] mx-auto" size={26} /> },
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="bg-white border border-gray-200 rounded-3xl p-8 shadow-sm">
          <div className="text-center mb-8 flex items-center justify-center gap-4">
            <div className="hidden sm:block h-[2px] w-16 bg-[#a442af]"></div>
            <h2 className="text-2xl font-black uppercase tracking-tight text-[#122a52]">We Solve Fitness Businesses' Growth Challenges</h2>
            <div className="hidden sm:block h-[2px] w-16 bg-[#a442af]"></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
            {["Low footfall & trial sign-ups","High competition from other gyms","Inconsistent membership growth","High cost per lead & low ROI","No-shows & last-minute cancellations","Lack of effective follow-up","Low visibility in local search","Difficulty tracking ad performance"].map((c, idx) => (
      ...
      <section className="max-w-7xl mx-auto px-4 py-12 text-center">
        <style>{`@keyframes mq-fit{0%{transform:translateX(0%)}100%{transform:translateX(-50%)}}.mq-fit{display:flex;width:max-content;animation:mq-fit 32s linear infinite}.mq-fit:hover{animation-play-state:paused}`}</style>
        <h3 className="text-slate-800 uppercase font-black text-sm tracking-widest mb-6">Trusted By Fitness Businesses</h3>
        <div className="relative w-full overflow-hidden py-2">
          <div className="mq-fit gap-6">
      ...
      <section className="max-w-7xl mx-auto px-4 pb-16">
        <div className="bg-[#a442af] text-white rounded-3xl p-8 md:p-12 text-center md:text-left flex flex-col md:flex-row justify-between items-center gap-6 shadow-xl">
          <div className="space-y-2">
            <h2 className="text-2xl md:text-3xl font-black uppercase tracking-tight">READY TO GROW YOUR FITNESS BUSINESS?</h2>
            <p className="font-bold text-purple-100">More Leads. More Trials. More Members. More Revenue. Book Your Free Lead Generation Strategy Session Today!</p>
          </div>
      ...
      <footer className="bg-[#122a52] text-white text-xs py-4 px-4 font-medium">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-3 text-center">
          <div className="flex flex-wrap justify-center gap-2 md:gap-4 text-slate-300">
            ...
            <span>|</span><span>Google Ads</span><span>|</span><span>Meta Ads</span><span>|</span><span>Instagram Ads</span><span>|</span><span>TikTok Ads</span><span>|</span><span>Cross-Channel Marketing</span>
          </div>
