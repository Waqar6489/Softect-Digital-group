...
import { CheckCircle, Phone, Mail, Heart, Activity, BarChart3 } from 'lucide-react';
import { FaGoogle, FaInstagram, FaTiktok, FaPhoneAlt, FaBullhorn, FaSync, FaSitemap } from "react-icons/fa";
import Logo from '../../Headers/logo';

export default function HealthcareLandingPage() {

  const trustItems = [
    { name: "Multi-Specialty Hospitals", icon: "🏥" },
    { name: "Clinics & Private Practices", icon: "🩺" },
    { name: "Dental & Orthodontic Centers", icon: "🦷" },
    { name: "Diagnostic & Imaging Centers", icon: "🔬" },
    { name: "Physiotherapy & Rehab Centers", icon: "🧑" },
    { name: "Wellness & Mental Health Clinics", icon: "🧠" },
    { name: "Pediatric & Child Care", icon: "👶" },
    { name: "Eye Care & Optical Centers", icon: "👁️" },
  ];
  ...
  const [formData, setFormData] = useState({
    ...
    practiceHospitalName: '', specialtyService: '', location: '', budget: '',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${apiBase}/api/healthcare-lead`, {
        method: 'POST',
      ...
      if (result.status === 'success') {
        ...
        setFormData({ name: '', email: '', phone: '', message: '', practiceHospitalName: '', specialtyService: '', location: '', budget: '' });
        setStep(1);
    ...
    <div className="bg-gray-50 text-gray-900 font-sans antialiased">
      ...
      <section className="max-w-7xl mx-auto px-4 py-12 lg:py-20 grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
        <div className="lg:col-span-7 space-y-6">
          ...
          <h1 className="text-4xl md:text-6xl font-black uppercase tracking-tight leading-none text-[#122a52]">
            More Leads.<br />More Patients.<br /><span className="text-[#a442af]">More Growth.</span>
          </h1>
          <p className="text-xl text-slate-700 font-medium max-w-2xl">
            Lead Generation Services for Healthcare Providers & Medical Practices using Google Ads, Meta Ads, TikTok Ads & Cross-Channel Marketing.
          </p>
          <div className="space-y-3 pt-4">
            {["Attract new patients actively searching for healthcare services","Increase appointment bookings & reduce no-shows","Fill your patient pipeline with high-quality leads","Maximize ROI with data-driven ad campaigns","Real-time tracking, reporting & campaign optimization"].map((text, idx) => (
        ...
        <div id='form' className="lg:col-span-5 border-4 border-[#a442af] rounded-2xl p-6 bg-white shadow-xl">
          <div className="text-center mb-6 pt-2">
            ...
            <h2 className="text-2xl font-black uppercase tracking-tight text-[#a442af]">Healthcare Leads</h2>
          </div>
          ...
          <form onSubmit={(e) => e.preventDefault()} className="space-y-4">
            ...
            {step === 2 && (
              <div className="space-y-4">
                {[
                  {label:'Practice / Hospital Name',id:'practiceHospitalName',type:'text'},
                  {label:'Specialty / Service',id:'specialtyService',type:'text'},
                  {label:'Location / Area Served',id:'location',type:'text'},
                  {label:'Monthly Ad Budget',id:'budget',type:'number'},
                ].map((f) => (
                  <div key={f.id} className="flex flex-col sm:flex-row sm:items-center gap-2 border-b border-gray-400 pb-1">
                    <label htmlFor={f.id} className="font-bold text-sm text-[#122a52] sm:w-1/3 whitespace-nowrap">{f.label}:</label>
                    <input type={f.type} id={f.id} value={formData[f.id]} placeholder={f.placeholder||''} className="w-full bg-transparent border-none focus:ring-0 text-sm py-1 px-5 outline-none text-slate-700" onChange={handleInputChange} />
                  </div>
                ))}
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100">
          <div className="lg:col-span-5 p-8 lg:p-12 space-y-6">
            <h3 className="text-[#a442af] uppercase font-black tracking-tight text-lg">Helping Healthcare Providers</h3>
            <h2 className="text-3xl font-black uppercase text-[#122a52] leading-tight">Attract More Patients. <span className="text-[#a442af]">Grow Your Practice.</span></h2>
            <p className="text-slate-600 font-medium">Softech helps hospitals, clinics, specialists, and healthcare providers generate high-intent leads through Google Ads, Meta Ads, TikTok Ads, and Cross-Channel Marketing.</p>
            <p className="text-slate-600 font-medium">Whether you run a multi-specialty hospital, private practice, diagnostic center, or wellness clinic — we help you reach the right patients at the right time and grow your practice.</p>
          ...
          <div className="lg:col-span-7 relative h-full min-h-[450px]">
            <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url('https://images.unsplash.com/photo-1559757148-5c350d0d3c56?q=80&w=1200&auto=format&fit=crop')` }}>
            ...
            <div className="absolute bottom-6 right-6 left-6 md:left-auto md:w-96 bg-white p-6 rounded-2xl shadow-2xl border-t-4 border-[#a442af] space-y-3">
              <div className="flex items-center gap-2">
                <div className="bg-[#a442af] text-white p-2 rounded-full"><Heart size={18} /></div>
                <h4 className="font-black uppercase tracking-tight text-sm text-[#122a52]">Helping Healthcare Providers Grow</h4>
              </div>
              <p className="text-xs text-slate-600 font-medium leading-relaxed">We generate high-quality patient leads that convert into appointments, treatments, and long-term patient relationships — improving your growth and ROI.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="text-center mb-8 flex items-center justify-center gap-4">
          ...
          <h2 className="text-2xl font-black uppercase tracking-tight text-[#122a52]">What Our Healthcare Clients Say</h2>
          <div className="hidden sm:block h-[2px] w-16 bg-[#a442af]"></div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { quote: "Softech increased our new patient inquiries by 58% within just 90 days through Google Ads and Meta Ads.", author: "HealthPlus Clinic" },
            { quote: "We saw a 3X increase in appointment bookings with high-intent leads from TikTok & cross-channel campaigns.", author: "VitalCare Multispecialty" },
            { quote: "Excellent reporting, great communication, and leads that actually convert into loyal patients.", author: "City Dental & Implant Center" }
          ].map((t, idx) => (
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="text-center mb-10 flex items-center justify-center gap-4">
          <div className="hidden sm:block h-[2px] w-16 bg-[#a442af]"></div>
          <h2 className="text-2xl font-black uppercase tracking-tight text-[#122a52]">Our Healthcare Lead Generation Services</h2>
          <div className="hidden sm:block h-[2px] w-16 bg-[#a442af]"></div>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4 text-center">
          {[
            { label: "Google Ads for Healthcare", icon: <FaGoogle className="text-[#a442af] mx-auto" size={28} /> },
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="bg-white border border-gray-200 rounded-3xl p-8 shadow-sm">
          <div className="text-center mb-8 flex items-center justify-center gap-4">
            <div className="hidden sm:block h-[2px] w-16 bg-[#a442af]"></div>
            <h2 className="text-2xl font-black uppercase tracking-tight text-[#122a52]">We Solve Healthcare Marketing Challenges</h2>
            <div className="hidden sm:block h-[2px] w-16 bg-[#a442af]"></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
            {["Low patient awareness & visibility","High cost per lead & poor ROI","Low appointment conversion rates","Competition from large hospitals & chains","No-shows & last-minute cancellations","Lack of follow-up & patient nurturing","Difficulty targeting the right patients","Inconsistent patient flow"].map((c, idx) => (
      ...
      <section className="max-w-7xl mx-auto px-4 py-12 text-center">
        <style>{`@keyframes mq-hc{0%{transform:translateX(0%)}100%{transform:translateX(-50%)}}.mq-hc{display:flex;width:max-content;animation:mq-hc 32s linear infinite}.mq-hc:hover{animation-play-state:paused}`}</style>
        <h3 className="text-slate-800 uppercase font-black text-sm tracking-widest mb-6">Trusted By Healthcare Providers</h3>
        <div className="relative w-full overflow-hidden py-2">
          <div className="mq-hc gap-6">
      ...
      <section className="max-w-7xl mx-auto px-4 pb-16">
        <div className="bg-[#a442af] text-white rounded-3xl p-8 md:p-12 text-center md:text-left flex flex-col md:flex-row justify-between items-center gap-6 shadow-xl">
          <div className="space-y-2">
            <h2 className="text-2xl md:text-3xl font-black uppercase tracking-tight">READY TO GROW YOUR HEALTHCARE PRACTICE?</h2>
            <p className="font-bold text-purple-100">Get More Patients. More Appointments. More Revenue. Book Your Free Healthcare Marketing Consultation Today.</p>
          </div>
      ...
      <footer className="bg-[#122a52] text-white text-xs py-4 px-4 font-medium">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-3 text-center">
          <div className="flex flex-wrap justify-center gap-2 md:gap-4 text-slate-300">
            <span className="font-bold text-white">🛡️ Healthcare Lead Generation Experts</span>
            <span>|</span><span>Google Ads</span><span>|</span><span>Meta Ads</span><span>|</span><span>TikTok Ads</span><span>|</span><span>Cross-Channel Marketing</span>
          </div>
